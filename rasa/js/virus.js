window.addEventListener('load', function () {
    // var address = 'http://175.178.7.180:8080/rasaProject/';
    var address = "";
    ...
    for (let b = 0; b < times.length; b++) {
        times[b].innerHTML = year + "-" + month + "-" + (day - 3 + b);
    ...
    const wzzgR = document.querySelector('#wzzgR');
    //传数据部分
    ...
    s.then(value => {
        ...
        bottoms[0].innerHTML = ss_array.chinaTotal.today.input == undefined ? "" : ss_array.chinaTotal.today.input;//今天境外输入
        centers[1].innerHTML = ss_array.chinaTotal.extData.noSymptom;//无症状感染者
        if (ss_array.chinaTotal.extData.incrNoSymptom == undefined) {
            wzzgR.innerHTML = '较昨日待公布';
        } else {
