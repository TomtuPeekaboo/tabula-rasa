...
window.addEventListener('load', function () {
        ...
        for (key in citys) {
                if (key.length == 2) {
                        options += '<option value="' + key + '">' + citys[key] + '</option>'
                }
        }
        document.getElementById("begin_pro").innerHTML = options;
        document.getElementById("end_pro").innerHTML = options;



        const begin_pro = document.querySelector("#begin_pro");
        const begin_city = document.querySelector("#begin_city");
        begin_pro.addEventListener("change", function () {
                // 获取选中的option对应的索引
                var index = begin_pro.options.selectedIndex;
                // 获取指定 index的option
                var opt = begin_pro.options[index]
                // 获取 val属性
                var pro_code = opt.value

                var options = "";
                for (key in citys) {
                        if (key.length == 4 && key.startsWith(pro_code)) {
                                options += '<option value="' + key + '">' + citys[key] + '</option>'
                        }
                }
                document.getElementById("begin_city").innerHTML = options;
        })

        const end_pro = document.querySelector("#end_pro");
        const end_city = document.querySelector("#end_city");
        end_pro.addEventListener("change", function () {
                // 获取选中的option对应的索引
                var index = end_pro.options.selectedIndex;
                // 获取指定 index的option
                var opt = end_pro.options[index]
                // 获取 val属性
                var pro_code = opt.value
                ...
                document.getElementById("end_city").innerHTML = options;
        })


        function findCityId(c) {
                for (var i = 0; i < json.length; i++) {
                        var pro = json[i];
                        for (var j = 0; j < pro.list.length; j++) {
                                var city = pro.list[j];
                                if (city.name == c) {
                                        return city.id
                                }
        ...
        //出发地
        const begin_name = document.querySelector("#begin_name");
        //外出政策
        const begin_policy = document.querySelector("#begin_policy p");
        //目的地
        const end_name = document.querySelector("#end_name");
        //目的地政策
        const end_policy = document.querySelector("end_policy p")
        console.log(end_policy)
        //返程
        const back_begin = document.querySelector("#back_begin");
        const back_begin_policy = document.querySelector("#back_begin_policy p");
        const back_end = document.querySelector("#back_end")
        const back_end_policy = document.querySelector("#back_end_policy p");
        ...
        guard.addEventListener("change", function () {
                ...
                for (key in citys) {
                        if (begin_city.value == key) {
                                begin_id = citys[key];
                                begin_id = begin_id.substring(0, begin_id.length - 1)
                                begin_id = findCityId(begin_id);
                        }
                }
                for (key in citys) {
                        if (end_city.value == key) {
                                end_id = citys[key];
                                end_id = end_id.substring(0, end_id.length - 1)
                                end_id = findCityId(end_id);
                        }
                }

                const policy = new XMLHttpRequest();
                //创建对象

                policy.open("get", "requestPolicyAPIServlet?cityid=" + begin_id + "," + end_id, true);
                policy.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                policy.send();
                policy.onreadystatechange = function () {
                        //判断
                        //200+都可以
                        if (policy.readyState === 4 && policy.status === 200) {
                                //保存数据

                                var json = JSON.parse(policy.responseText)

                                //正确的数据

                                console.log(json)

                                // let begin=JSON.parse(json.data[0]);
                                // let end=JSON.parse(json.data[1]);

                                begin_name.innerHTML = "出发地：" + json.data[0].city;
                                end_name.innerHTML = "目的地：" + json.data[1].city;
                                begin_policy.innerHTML = json.data[0].leave_policy;
                                back_begin.innerHTML = "出发地：" + json.data[1].city;
                                back_end.innerHTML = "目的地：" + json.data[0].city;

                                back_begin_policy.innerHTML = json.data[1].leave_policy;
                                back_end_policy.innerHTML = json.data[0].back_policy;
                                console.log(json.data[1].back_policy)
                                end_policy.innerHTML = json.data[1].back_policy;
